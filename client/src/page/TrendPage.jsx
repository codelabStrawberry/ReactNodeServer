    ...
    const fetchJobTrend = async () => {
      ...
      try {
        ...
        const res = await axios.post(
          `${import.meta.env.VITE_AI_URL}/trend/jobfit`,
          { job_cat: jobName },
        )
        if (res.data.error) {
  ...
  useEffect(() => {
    ...
    const fetchCareerStrategy = async () => {
      ...
      try {
        const jobName = SARAMIN_CATEGORY[selectedJob]
        const res = await axios.post(
          `${import.meta.env.VITE_AI_URL}/trend/career_advice`,
          { job_cat: jobName },
        )
  ...
  return (
    <Container>
      ...
      <TitleWrapper>
        ...
        <Description>
          내 직군에 필요한 역량을 최신 트렌드로 업데이트하고, 다음 준비를 더
          정확하게 설계하세요.
        </Description>
      </TitleWrapper>
      <JobAnalysis>
        ...
        <ChartWrapper>
          ...
          <JobTrendChart>
            ...
            {selectedJob === "0" && (
              <Placeholder>
                직군을 선택하여 최신 트렌드를 확인하세요.
              </Placeholder>
            )}
            {isLoadingJobfit && (
              <p>{SARAMIN_CATEGORY[selectedJob]} 데이터 분석 중...</p>
            )}
      ...
      <CareerWrapper>
        ...
        <CareerContext>
          ...
          {selectedJob === "0" && (
            <Placeholder>직군을 선택하여 커리어 전략을 확인하세요.</Placeholder>
          )}
          {selectedJob !== "0" && isLoadingCareer && (
            <p>
              선택한 직군: {SARAMIN_CATEGORY[selectedJob]}에 맞춘 커리어 전략
              분석 중...
            </p>
          )}
